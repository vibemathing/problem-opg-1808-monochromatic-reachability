// R03 candidate search: physical-path vertex filtering and dominator propagation.
// Derived from the R02 source at base 489ae8ca7ef47db13716b9fe752828f9e67b7476.
...
uint64_t path_reject=0,via_forced=0;
...
Bits reach_in(int start, Bits mask,const Bits *adj){
 Bits reached=1u<<start,previous=0;
 while(reached!=previous){previous=reached;for(int u=0;u<n;++u)if(reached&(1u<<u))reached|=adj[u]&mask;}
 return reached;
}
bool propagate(State&s,vector<Domain>&domains){
 ...
 for(;;){
  ...
  Bits physical[K][M]{};for(int c=0;c<k;++c)for(int u=0;u<n;++u)physical[c][u]=hi[c][u];
  close(hi);
  ...
  if(!changed&&mode==3){
   for(int c=0;c<k;++c)for(int u=0;u<n;++u)for(int v=0;v<n;++v){
    if(u==v||!(s.lo[c][u]&(1u<<v)))continue;
    Bits mask=0;
    for(int x=0;x<n;++x){State t,t2;if(add_reach(s,u,x,c,t)&&add_reach(t,x,v,c,t2))mask|=1u<<x;}
    if(!(reach_in(u,mask,physical[c])&(1u<<v))){++path_reject;return false;}
    for(int x=0;x<n;++x){
     if(x==u||x==v||!(mask&(1u<<x)))continue;
     if((s.lo[c][u]&(1u<<x))&&(s.lo[c][x]&(1u<<v)))continue;
     if(!(reach_in(u,mask&~(1u<<x),physical[c])&(1u<<v))){
      State t,t2;if(!add_reach(s,u,x,c,t)||!add_reach(t,x,v,c,t2)){++path_reject;return false;}
      s=t2;changed=true;++via_forced;
     }
    }
   }
  }
...
int main(int argc,char**argv){try{
 ...
 if(n<3||n>M||k<1||k>K||(nr!=0&&nr!=1)||mode<0||mode>3||seconds_limit<=0||!node_limit||int(word.size())!=n)throw invalid_argument("invalid domain");
 ...
 if(ok)dfs(s);
 cout<<"{\"n\":"<<n<<",\"palette\":"<<k<<",\"no_rainbow\":"<<(nr?"true":"false")<<",\"mode\":"<<mode<<",\"cycle_word\":\""<<word<<"\",\"status\":\""<<(stopped?"incomplete":found?"witness":"exhausted_no_witness")<<"\",\"nodes\":"<<nodes<<",\"trials\":"<<trials<<",\"triangle_reject\":"<<triangle_reject<<",\"lower_reject\":"<<lower_reject<<",\"empty_domain\":"<<empty_domain<<",\"upper_reject\":"<<upper_reject<<",\"forced_reach\":"<<forced_reach<<",\"rounds\":"<<rounds<<",\"path_reject\":"<<path_reject<<",\"via_forced\":"<<via_forced<<",\"leaves\":"<<leaves<<",\"elapsed_seconds\":"<<elapsed()<<",\"arcs\":[";
