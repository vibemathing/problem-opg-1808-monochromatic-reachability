...
int main(int argc,char**argv){
 try {
 if(argc!=6)throw invalid_argument("arguments: N COLOURS NO_RAINBOW TIMEOUT_SECONDS NODE_LIMIT");
 ...
 start=chrono::steady_clock::now();dfs(s);
 double elapsed=chrono::duration<double>(chrono::steady_clock::now()-start).count();
 cout<<"{\"n\":"<<n<<",\"palette\":"<<k<<",\"no_rainbow\":"<<(nr?"true":"false")<<",\"status\":\""<<(stopped?"incomplete":found?"witness":"exhausted_no_witness")<<"\",\"nodes\":"<<nodes<<",\"trials\":"<<trials<<",\"triangle_rejections\":"<<prune_triangle<<",\"reach_rejections\":"<<prune_reach<<",\"dead_nodes\":"<<dead<<",\"leaves\":"<<leaves<<",\"elapsed_seconds\":"<<elapsed<<",\"arcs\":[";
